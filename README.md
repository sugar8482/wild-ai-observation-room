...
- **多聊天室**：嘉宾属于全局嘉宾库，同一位嘉宾可加入多个房间；每个房间独立保存成员关系、聊天记录、房间氛围和公开长期总结
...
- **角色私人记忆**：每位嘉宾可独立开关；开启后用同一次回复悄悄追加第一人称心事、偏向或猜测，并可跟随同一位嘉宾跨房间、跨普通聊天与游戏模式延续
...
- **数据库主存储**：下载版自动使用零配置 SQLite；配置 PostgreSQL 后自动切换为 PostgreSQL。JSON 只保留为兼容与灾备镜像
...
新建聊天室时把“房间模式”选成“狼人杀游戏房”，再选择本房嘉宾即可。游戏房不会改动普通聊天室的发言、长期总结和后台定时设置；每局身份、夜间行动、狼队密谈、悍跳和倒钩都保存在独立卷宗里，散场后才解锁完整复盘。嘉宾会带着自己的长期私人记忆入场，让已有的信任、怀疑和偏向自然影响玩法，但对局期间记忆严格只读，不能新增、修改、归档或把记忆便笺发上公屏；旧局原文、本局日记和身份也不会灌进下一局。只有游戏结束进入赛后复盘后，嘉宾才可以从本局挑选少量真正值得长期保留的认识。

- **玩家模式**：曦曦与 5～6 位 AI 同局，随机抽取狼人、预言家、女巫或村民；只能看到自己依法拥有的秘密，也可能被 AI 怀疑、投票或夜里刀掉
- **上帝模式**：曦曦不占身份牌，选择 6～7 位 AI 后开全视野围观，可以亲手指定每个人的身份或交给系统随机洗牌，并看到狼队密谈、验人和用药过程
...
不配置数据库时，观察室会自动创建 `data/observation-room.sqlite`，无需安装数据库、填写连接地址或执行迁移；因此从 GitHub 下载项目的用户仍然可以直接启动。SQLite 是本地单机主存储，`data/state.json` 同时保留为兼容和灾备镜像。
...
启动后程序会自动建表，并在数据库尚为空时把现有 JSON 完整迁入。以后 PostgreSQL 是服务端主状态来源；每次保存先写数据库，再更新 JSON 灾备镜像。已有房间、聊天、嘉宾、人设、头像、私人记忆和狼人杀卷宗不会因切换存储而重建。

数据库保存：全局嘉宾目录、房间与成员关系、房间完整消息（含私聊收件人字段）、长期总结及历史版本、每位嘉宾的全局私人记忆，以及狼人杀的整局状态、逐条事件和本局私人日记。嘉宾与房间是多对多关系：从一个房间离席不会删除嘉宾，也不会删除她在其他房间的席位或私人记忆。API Key 和额外请求头只会以状态存储中原有的加密形式保存；访问码与会话令牌不会进入关系档案表。

## 数据与隐私

- API Key 只在填写时从浏览器发送给本机服务，之后以 AES-256-GCM 加密保存在主数据库状态中，并同步到 `data/state.json` 灾备镜像
- 加密密钥保存在 `.env.local`；刷新页面时，服务不会把 API Key 明文返回浏览器
- 聊天室、嘉宾配置、聊天记录和房间摘要保存在 SQLite 或 PostgreSQL 主数据库中，并同步保留 JSON 灾备镜像
- `data/`、`.env.local`、构建产物与运行文件都已被 Git 排除，不会随正常提交上传

本地版请同时备份 `.env.local`、`data/observation-room.sqlite` 和 `data/state.json`；PostgreSQL 版请备份 `.env.local`、数据库和 JSON 灾备镜像。丢失加密密钥后，已经保存的 API Key 无法恢复。
