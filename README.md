...
自动新增记忆会先在本地清理完全重复、重复日期标签和同一事件的换句复述，只优先保留新变化、重要误会、关系转折及未公开秘密；超过约 9,000 字时先合并旧内容，再淘汰较旧的普通碎片，不额外调用模型。编辑窗口里的“本地瘦身”可以随时整理已有内容，文本仍可手动修改或删除。若本地规则仍无法理清复杂旧账，可手动点“深度整理”；页面会明确确认后才调用一次已配置的记忆整理员，结果先作为可检查的草稿，不会自动覆盖保存。
