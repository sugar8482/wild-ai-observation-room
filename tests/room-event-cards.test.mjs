...
test("生活事件卡会避开最近四张且不会擅自编造用户状态", () => {
  ...
  assert.match(roomEventCardPrompt(card), /具体内容绝不能跨房间照搬/);
  assert.match(roomEventCardPrompt(card, { focus: "只写古代客栈里合理发生的事。" }), /只写古代客栈里合理发生的事/);
  assert.match(roomEventCardPrompt(card, {}, { name: "狼人杀" }), /房间“狼人杀”/);
});

test("使用过的事件卡会进入近期记录并保持固定窗口", () => {
  let state = { enabled: true, focus: "只写本房事件。", recentIds: [], lastEvent: "", revision: 0 };
  ...
  assert.equal(state.focus, "只写本房事件。");
