...
export function createStateStore({ filePath, secret }) {
  ...
  async function publicRoomSnapshot(roomId, {
    ...
    const participantNames = room.participantIds
      .map((id) => current.agents.find((agent) => agent.id === id)?.name)
      .filter(Boolean);
    const snapshot = {
      id: room.id,
      name: room.name,
    ...
    if (includeContext) {
      ...
      snapshot.privateRecipients = [
        ...
        ...room.participantIds.map((id) => {
          ...
          return agent ? { id, protocolId: id, name: agent.name, type: "agent" } : null;
        }).filter(Boolean),
      ];
    }
    return snapshot;
  ...
  async function appendExternalMessage(roomId, {
    ...
    const task = async () => {
      ...
      const allowedPrivateRecipients = new Set([ROOM_USER_ID, ...room.participantIds]);
