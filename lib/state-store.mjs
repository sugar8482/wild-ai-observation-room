...
import { sanitizeWerewolfArchives, sanitizeWerewolfGame } from "../public/werewolf-game.js";
...
function sanitizeRoomMemory(memory) {
  return {
    ...
    interval: boundedInteger(memory?.interval, DEFAULT_ROOM_SUMMARY_INTERVAL, 5, 100),
    recentMessages: boundedInteger(memory?.recentMessages, 30, 10, 80),
    focus: text(memory?.focus, 20_000),
    summary: text(memory?.summary, ROOM_SUMMARY_STORAGE_LIMIT),
    summarizedThroughId: cleanId(memory?.summarizedThroughId),
    summarizedMessageCount: boundedInteger(memory?.summarizedMessageCount, 0, 0, Number.MAX_SAFE_INTEGER),
...
function sanitizeRoom(room, validAgentIds) {
  ...
  return {
    ...
    hiddenExternalMemberIds: [...new Set((Array.isArray(room?.hiddenExternalMemberIds) ? room.hiddenExternalMemberIds : [])
      .map(cleanId)
...
function newerWerewolfGame(left, right) {
  ...
  return (Number(left.updatedAt) || 0) >= (Number(right.updatedAt) || 0) ? left : right;
}

function mergeWerewolfState(next, current, incomingRoom) {
  if (next.roomType !== "werewolf") return;
  const archivesById = new Map();
  for (const archived of [...(current.werewolfArchives || []), ...(next.werewolfArchives || [])]) {
    if (!archived?.id) continue;
    archivesById.set(archived.id, newerWerewolfGame(archivesById.get(archived.id), archived));
  }
  next.werewolfArchives = [...archivesById.values()]
    .sort((left, right) => (Number(left.archivedAt) || 0) - (Number(right.archivedAt) || 0));

  const incomingHasWerewolf = Object.hasOwn(incomingRoom || {}, "werewolf");
  const incomingGame = next.werewolf;
  const currentGame = current.werewolf;
  const archivedIds = new Set(next.werewolfArchives.map((game) => game.id));
  if (incomingGame && archivedIds.has(incomingGame.id)) {
    next.werewolf = currentGame && !archivedIds.has(currentGame.id) ? currentGame : null;
  } else if (incomingGame && currentGame && incomingGame.id === currentGame.id) {
    next.werewolf = newerWerewolfGame(incomingGame, currentGame);
  } else if (incomingGame) {
    next.werewolf = incomingGame;
  } else if (!incomingHasWerewolf || (currentGame && !archivedIds.has(currentGame.id))) {
    next.werewolf = currentGame || null;
  } else {
    next.werewolf = null;
  }
}
...
function mergeRoomForSave(incomingRoom, currentRoom, validAgentIds, now = Date.now()) {
  ...
  const currentSanitized = sanitizeRoom(currentRoom, validAgentIds);
  mergeWerewolfState(next, currentSanitized, incomingRoom);
  const currentSchedule = sanitizeRoomSchedule(currentRoom.schedule);
  const currentMembers = currentSanitized.members;
  ...
  next.hiddenExternalMemberIds = incomingExternalRevision >= currentExternalRevision
    ? [...next.hiddenExternalMemberIds]
    : [...currentSanitized.hiddenExternalMemberIds];
  const hiddenExternalMemberIds = new Set(next.hiddenExternalMemberIds);
  next.members = next.members.filter((member) => member.type === "agent" || !hiddenExternalMemberIds.has(member.id));
...
export function createStateStore({ filePath, secret, database = null, onStateChange = null }) {
  ...
  async function load() {
    ...
    if (typeof database?.loadState === "function") {
      try {
        const stored = await database.loadState();
        if (stored && typeof stored === "object") {
          state = stored;
          loadedFromDatabase = true;
        }
      } catch (error) {
        // Do not silently fall back to an older JSON mirror when the primary
        // database is merely unavailable. That could create two divergent
        // timelines and later overwrite newer database data.
        throw new Error(`主数据库暂时无法读取：${error?.message || "未知错误"}`);
      }
    }
    if (!state) {
      try {
    ...
    if (!loadedFromDatabase && typeof database?.saveState === "function") {
      try {
        await database.saveState(state);
      } catch {
        // Startup can still recover from the JSON mirror; subsequent writes remain fail-closed.
      }
    }
    return state;
  }
  ...
  async function writeState() {
    if (typeof database?.saveState === "function") await database.saveState(state);
    ...
    const task = async () => {
      ...
      for (const agent of incomingAgents) {
        ...
        nextAgents.push({
          ...
          avatar: Object.hasOwn(agent || {}, "avatar") ? avatarData(agent?.avatar) : avatarData(existing?.avatar),
  ...
  async function completeRoomSummary(roomId, {
    ...
    const task = async () => {
      ...
      if (nextSummary.length > ROOM_SUMMARY_STORAGE_LIMIT) {
        throw new Error(`房间长期总结超过安全保存上限 ${ROOM_SUMMARY_STORAGE_LIMIT} 字，整理锚点未推进`);
      }
      memory.summary = nextSummary;
      memory.summarizedThroughId = cleanId(summarizedThroughId);
      memory.summarizedMessageCount = boundedInteger(
        summarizedMessageCount,
        0,
        0,
  ...
  async function publicRoomSnapshot(roomId, {
    ...
    const cleanAfterMessageId = cleanId(afterMessageId);
    const afterMessageIndex = cleanAfterMessageId
      ? visibleMessages.findIndex((message) => message.id === cleanAfterMessageId)
      : -1;
    const incremental = afterMessageIndex >= 0 || afterTimestamp > 0;
    const pendingMessages = afterMessageIndex >= 0
      ? visibleMessages.slice(afterMessageIndex + 1)
      : visibleMessages.filter((message) => Number(message.timestamp) > afterTimestamp);
    const selectedMessages = incremental
      ? pendingMessages.slice(0, safeLimit)
      : pendingMessages.slice(-safeLimit);
    ...
    const snapshot = {
      ...
      members,
      messages,
      nextMessageId: messages.at(-1)?.id || cleanAfterMessageId || null,
      nextAfter: messages.length
        ? Math.max(...messages.map((message) => Number(message.timestamp) || 0))
        : afterTimestamp,
      hasMore: incremental && pendingMessages.length > selectedMessages.length,
      latestVisibleMessageId: visibleMessages.at(-1)?.id || null,
      externalRevision: boundedInteger(room.externalRevision, 0, 0, Number.MAX_SAFE_INTEGER),
      updatedAt: Number(room.updatedAt) || Date.now(),
    };
    if (includeContext) {
      const summaryStale = isLegacyTruncatedRoomSummary(room.memory);
      snapshot.roomPrompt = text(room.roomPrompt, 20_000);
      snapshot.longTermSummary = summaryStale
        ? ""
        : text(room.memory?.summary, ROOM_SUMMARY_STORAGE_LIMIT);
      snapshot.summaryStale = summaryStale;
      snapshot.summaryUpdatedAt = Number(room.memory?.updatedAt) || null;
  ...
  async function deleteWerewolfEvent(roomId, gameId, eventId) {
    const task = async () => {
      ...
      if (!room || room.roomType !== "werewolf" || !safeGameId || !safeEventId) {
        return { gameFound: false, deleted: false, snapshot: await clientState() };
      }
      const games = [
        room.werewolf,
        ...(Array.isArray(room.werewolfArchives) ? room.werewolfArchives : []),
      ].filter(Boolean);
      const game = games.find((item) => item.id === safeGameId);
      if (!game) return { gameFound: false, deleted: false, snapshot: await clientState() };
      const originalLog = Array.isArray(game.log) ? game.log : [];
      const nextLog = originalLog.filter((entry) => entry.id !== safeEventId);
      if (nextLog.length === originalLog.length) {
        return { gameFound: true, deleted: false, snapshot: await clientState() };
      }
      game.log = nextLog;
      game.revision = Math.max(1, Number(game.revision) || 1) + 1;
      game.updatedAt = Date.now();
      room.updatedAt = game.updatedAt;
      await writeState();
      return { gameFound: true, deleted: true, snapshot: await clientState() };
    };
    writeQueue = writeQueue.then(task, task);
    return writeQueue;
  }

  async function saveWerewolfGame(roomId, incomingGame) {
    const task = async () => {
      const current = await load();
      const room = current.rooms.find((item) => item.id === cleanId(roomId));
      if (!room || room.roomType !== "werewolf") return null;
      const game = sanitizeWerewolfGame(incomingGame);
      if (!game) return null;
      const stored = sanitizeWerewolfGame(room.werewolf);
      const archivedIds = new Set((room.werewolfArchives || []).map((item) => item?.id).filter(Boolean));
      if (archivedIds.has(game.id)) return game;
      if (stored && stored.id !== game.id && Number(stored.createdAt) > Number(game.createdAt)) {
        return stored;
      }
      room.werewolf = stored?.id === game.id ? newerWerewolfGame(game, stored) : game;
      room.updatedAt = Math.max(Number(room.updatedAt) || 0, Number(room.werewolf.updatedAt) || Date.now());
      await writeState();
      return sanitizeWerewolfGame(room.werewolf);
    };
    writeQueue = writeQueue.then(task, task);
    return writeQueue;
  }
  ...
  async function editWerewolfEvent(roomId, gameId, eventId, messageText) {
    const task = async () => {
      ...
      const safeGameId = cleanId(gameId);
      const safeEventId = cleanId(eventId);
      const safeText = text(messageText, 50_000).trim();
      if (!room || room.roomType !== "werewolf" || !safeGameId || !safeEventId) {
        return { gameFound: false, eventFound: false, updated: false, text: safeText, snapshot: await clientState() };
      }
      const games = [
        room.werewolf,
        ...(Array.isArray(room.werewolfArchives) ? room.werewolfArchives : []),
      ...
      const game = games.find((item) => item.id === safeGameId);
      if (!game) return { gameFound: false, eventFound: false, updated: false, text: safeText, snapshot: await clientState() };
      const event = (Array.isArray(game.log) ? game.log : []).find((entry) => entry.id === safeEventId);
      if (!event) return { gameFound: true, eventFound: false, updated: false, text: safeText, snapshot: await clientState() };
      if (event.text === safeText) {
        return { gameFound: true, eventFound: true, updated: false, text: safeText, snapshot: await clientState() };
      }
      event.text = safeText;
      game.revision = Math.max(1, Number(game.revision) || 1) + 1;
      game.updatedAt = Date.now();
      room.updatedAt = game.updatedAt;
      await writeState();
      return { gameFound: true, eventFound: true, updated: true, text: safeText, snapshot: await clientState() };
    };
    writeQueue = writeQueue.then(task, task);
    return writeQueue;
  }
  ...
  async function setRoomMemberPresence(roomId, {
    ...
    const task = async () => {
      ...
      const validAgentIds = new Set(current.agents.map((agent) => agent.id));
      const requestedStatus = ["active", "away", "left"].includes(status) ? status : "";
      if (safeType !== "agent" && (requestedStatus !== "left" || touch)) {
        room.hiddenExternalMemberIds = (room.hiddenExternalMemberIds || []).filter((item) => cleanId(item) !== id);
      }
  ...
  async function removeExternalRoomMember(roomId, memberId) {
    const task = async () => {
      const current = await load();
      const room = current.rooms.find((item) => item.id === cleanId(roomId));
      const id = cleanId(memberId);
      if (!room || !id) return { roomFound: Boolean(room), memberFound: false, removed: false };
      const validAgentIds = new Set(current.agents.map((agent) => agent.id));
      const members = sanitizeRoomMembers(room, room.participantIds || [], validAgentIds);
      const member = members.find((item) => item.id === id);
      if (!member) return { roomFound: true, memberFound: false, removed: false };
      if (!["mcp", "human"].includes(member.type) || member.status !== "left") {
        return { roomFound: true, memberFound: true, removed: false, reason: "only-left-external" };
      }
      room.members = members.filter((item) => item.id !== id);
      room.hiddenExternalMemberIds = [...new Set([
        ...(Array.isArray(room.hiddenExternalMemberIds) ? room.hiddenExternalMemberIds : []),
        id,
      ].map(cleanId).filter(Boolean))].slice(-100);
      room.externalRevision = boundedInteger(room.externalRevision, 0, 0, Number.MAX_SAFE_INTEGER) + 1;
      room.updatedAt = Date.now();
      await writeState();
      return {
        ...
        roomId: room.id,
        memberId: id,
        externalRevision: room.externalRevision,
      };
    };
    writeQueue = writeQueue.then(task, task);
    return writeQueue;
