...
export async function editWerewolfEvent(pool, { roomId = "", gameId = "", eventId = "", text = "" } = {}) {
  const game = clean(gameId);
  const room = clean(roomId);
  const event = clean(eventId);
  const body = clean(text).trim();
  if (!game || !room || !event || !body) return { gameFound: false, eventFound: false, updated: false, text: body };
  const found = await gameRow(pool, game, room);
  if (!found) return { gameFound: false, eventFound: false, updated: false, text: body };
  const result = await pool.query(`
    UPDATE werewolf_events
    SET body = $4, persisted_at = now()
    WHERE game_id = $1 AND room_id = $2 AND event_id = $3
    RETURNING body
  `, [game, room, event, body]);
  return {
    gameFound: true,
    eventFound: Number(result.rowCount) > 0,
    updated: Number(result.rowCount) > 0,
    text: clean(result.rows?.[0]?.body || body),
  };
}
