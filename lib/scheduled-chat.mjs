...
import { buildAppendSummaryMessages, formatMemorySegment } from "../public/memory-prompt.js";
...
export async function runScheduledRoom({ room, agents, chat, random = Math.random, at = Date.now() }) {
  ...
  for (let turn = 1; turn <= room.schedule.maxTurns; turn += 1) {
    ...
    const winner = pickMicWinner(scores, micOptions, random);
  ...
  return { messages: generated, result, mic };
}
