...
const SCHEDULED_VISIBLE_TOKENS = DEFAULT_VISIBLE_REPLY_TOKENS;
...
function privateParticipants(agents, room) {
  return [...agents, ...externalMcpParticipants(room)];
}
...
function recentTranscript(room, agent, agents, limit = 40) {
  return visibleMessagesForAgent(room.messages, agent.id)
    ...
    .map((message) => {
      ...
      return `${when}${formatMessageForAgent(message, agent, privateParticipants(agents, room))}`;
    })
    .join("\n\n");
}
  ...
  return [
    {
      ...
      content: [
        ...
        roomPresenceContext(room, agents),
        `【房间共同氛围】\n${atmosphere}`,
        `【你的个人设定】\n${persona}`,
  ...
  return [
    {
      ...
      content: [
        ...
        room.bubbleSplit
          ...
          : `这是群聊中的一次发言。最终正文最多约 ${visibleTokenTarget} tokens；这是安全上限，不必写满，短答可以自然结束，优先保证句子完整。`,
        bubbleSplitInstruction(room.bubbleSplit),
        privateMessageOutputInstruction(agent, privateParticipants(agents, room)),
...
export async function runScheduledRoom({ room, agents, chat, random = Math.random, at = Date.now() }) {
  const activeAgents = activeRoomAgents(room, agents).filter(isConfigured);
  ...
  for (let turn = 1; turn <= room.schedule.maxTurns; turn += 1) {
    ...
    try {
      ...
      const parsedMessages = parsePrivateMessageReply(parsedReply.visibleText, {
        agentId: speaker.id,
        recipients: privateParticipants(activeAgents, workingRoom),
      });
...
export async function maybeSummarizeRoom(stateStore, chat, roomId) {
  ...
  if (!room?.memory?.enabled || !isConfigured(summarizer)) return false;
  if (isLegacyTruncatedRoomSummary(room.memory)) return false;
  ...
  const pending = messages.slice(previousMarker && markerIndex >= 0 ? markerIndex + 1 : fallbackCount);
  const source = completeAutomaticSummaryBatch(pending, room.memory.interval);
  if (!source.length) return false;

  const chunkSize = Math.max(5, Number(room.memory.interval) || DEFAULT_ROOM_SUMMARY_INTERVAL);
  let currentSummary = String(room.memory.summary || "").trim();
  let expectedMarker = previousMarker;
  let expectedUpdatedAt = room.memory.updatedAt;
  for (let index = 0; index < source.length; index += chunkSize) {
    ...
    const response = await chat({
      ...
      messages: buildAppendSummaryMessages(room, chunk, currentSummary),
    });
    currentSummary = String(response?.text || "").trim();
    if (!currentSummary) throw new Error("整理模型没有返回文字");
    const lastId = chunk.at(-1).id;
    const at = Date.now();
    const saved = await stateStore.completeRoomSummary(roomId, {
      summary: currentSummary,
      summarizedThroughId: lastId,
      summarizedMessageCount: messages.findIndex((message) => message.id === lastId) + 1,
      expectedPreviousMarker: expectedMarker,
      expectedPreviousUpdatedAt: expectedUpdatedAt,
      at,
    });
    if (!saved) return false;
    expectedMarker = lastId;
    expectedUpdatedAt = at;
  }
  return true;
}

export function createRoomScheduler({
  stateStore,
  chat,
  tickMs = SCHEDULER_TICK_MS,
  now = Date.now,
  isSummaryRunning = () => false,
  startAutomaticSummary = null,
}) {
  ...
  async function processRoom(room, agents, currentTime) {
    ...
    try {
      ...
      if (typeof startAutomaticSummary === "function") {
        try {
          await startAutomaticSummary(room.id);
        } catch {
          // Scheduled chat stays saved even when the optional memory editor is unavailable.
        }
      } else if (!isSummaryRunning(room.id)) {
        try {
